      ...
      <v-layout row wrap>
        ...
        <v-flex lg6 md6>
          <v-layout
            ...
            wrap
            style="margin:5px"
            v-if="flightReservation.flights.returnFlight !== null"
          >
            <v-flex lg12 md12>
              <h3>Return flight</h3>
            </v-flex>
            ...
            <v-stepper v-model="stepper">
              <v-stepper-header>
                <v-stepper-step :complete="stepper > 1" step="1"
                  >Pick a seats</v-stepper-step
                >
                <v-divider></v-divider>
                <v-stepper-step :complete="stepper > 2" step="2"
                  >Invite friends</v-stepper-step
                >
                <v-divider></v-divider>
                <v-stepper-step :complete="stepper > 3" step="3"
                  >Fill passengers information</v-stepper-step
                >
                <v-divider></v-divider>
              ...
              <v-stepper-items>
                <v-stepper-content step="1">
                  <seats
                    ...
                    v-bind:passengersNumber="
                      flightReservation.searchParams.passengersNumber
                    "
                ...
                <v-stepper-content step="2">
                  ...
                  <v-btn flat @click="stepper = 1" style="float:right"
                    >Back</v-btn
                  >
                </v-stepper-content>
                ...
                <v-stepper-content step="3">
                  <fill-passengers-info
                    ...
                    v-bind:airlineCompany="
                      flightReservation.flights.departureFlight.airlineCompany
                        .id
                    "
                  ></fill-passengers-info>

                  <v-btn color="primary" @click="reserve" style="float:right">
                    Next
                  </v-btn>

                  <v-btn flat @click="goToStepTwoOrOne" style="float:right"
                    >Back</v-btn
                  >
                </v-stepper-content>
...
export default {
  ...
  computed: {
    ...
    getReturnSeats() {
      if (this.flightReservation.flights.returnFlight === null) return null;
    ...
    enableBtnStep1() {
      ...
      if (seatsPickedDeparture === num) {
        if (this.flightReservation.flights.returnFlight === null) return true;
        else if (seatsPickedReturn === num) return true;
      }
      return false;
    }
  },
  created() {
    ...
    if (fr !== null) this.flightReservation = fr;
    else this.$router.push({ name: "flights" });

    UserController.getFriends(store.getters.activeUser.id).then(response => {
      store.commit("setFriends", response.data);
    });
    store.commit(
      "setPassengersDefault",
      this.flightReservation.searchParams.passengersNumber
    );
    this.getMyInfo();
  },
  methods: {
    ...
    goToFriendsInvitation() {
      this.forceRerender();
      if (
        store.getters.friends.length === 0 ||
        this.flightReservation.searchParams.passengersNumber === 1
      ) {
        this.friends = false;
        this.stepper = 3;
      } else {
    ...
    goToStepTwoOrOne() {
      if (this.friends) this.stepper = 2;
      else this.stepper = 1;
    },
    getMyInfo() {
    ...
    setInvitedFriends(data) {
      if (data.length === 0) this.invitedFriends = [];
      else {
        this.invitedFriends = [];
    ...
    goToStepThree() {
      ...
      for (let element of this.invitedFriends) {
        promises.push(UserController.getUser(element.id));
      }
    ...
    reserve() {
      ...
      for (let passenger of passengers) {
        if (
          ...
          store.commit("setSnack", {
            msg: "You must fill information about every passenger!",
            color: "primary"
          });
          return;
        }
      ...
      if (
        ...
        myInfo.address === "" ||
        myInfo.passport === "" ||
        myInfo.passport === null ||
        myInfo.phone === ""
      ) {
        store.commit("setSnack", {
          msg: "You must fill your information !",
          color: "primary"
        });
        return;
      }
      ...
      FlightController.reserve(data)
        .then(response => {
          loader.hide();
          store.commit("setSnack", {
            msg: "You have successfully booked a flight!",
            color: "success"
          });
          this.optional = true;
        })
