      ...
      <v-flex lg8 md8 sm6 xs12>
        <v-card>
          ...
          <v-card-actions>
            ...
            <v-btn
              color="primary"
              v-if="isGuest && isUser && !show && !isFriend"
              @click="addFriend"
            ...
            <v-btn
              ...
              v-if="isGuest && isUser && show && !isFriend"
              @click="withdraw"
              >Withdraw friend request</v-btn
            >
            <v-btn
              color="primary"
              v-if="isGuest && isUser && isFriend"
              @click="remove"
              >Remove friend</v-btn
            >
          </v-card-actions>
...
export default {
  ...
  methods: {
    getUser() {
      ...
      if (this.id == store.getters.activeUser.id) this.isGuest = false;
    ...
    addFriend() {
      ...
      FriendshipController.addFriend(data)
        .then(response => {
          store.commit("setSnack", {
            msg: "You successfully sent friend request",
            color: "success"
          });
          this.show = true;
        })
