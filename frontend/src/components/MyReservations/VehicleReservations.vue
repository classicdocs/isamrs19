<template>
  <div>
    <v-data-table :items="reservations" :headers="headers" class="elevation-1">
      <template v-slot:items="props">
        <td>
          {{ props.item.vehicle.vehicleManufacturer }}
          {{ props.item.vehicle.vehicleModel }}
        </td>
        <td>{{ props.item.vehicle.vehicleType }}</td>
        <td>
          {{ props.item.reservedFrom | moment("dddd, MMMM Do YYYY, HH:mm") }}
        </td>
        <td>
          {{ props.item.reservedUntil | moment("dddd, MMMM Do YYYY, HH:mm") }}
        </td>
        ...
        <td>
          <v-btn
            ...
            color="blue darken"
            flat
            @click="rate(props.item.id)"
            >Rate</v-btn
          >
        </td>
        <td>
          <v-btn color="error" flat @click="cancel(props.item)"
    ...
    <v-dialog width="300" v-model="ratingDialog">
      <v-card class="elevation-16 mx-auto">
        ...
        <v-card-text>
          If you enjoyed using our service, please take a few seconds to rate
...
export default {
  ...
  methods: {
    getData() {
      VehicleReservationController.get(store.getters.activeUser.id).then(
        response => {
    ...
    cancel(reservation) {
      VehicleCancelController.cancel(reservation.id)
        .then(response => {
          store.commit("setSnack", {
            msg: "You have successfully canceled a reservation.",
            color: "success"
          });
