<template>
  <div>
    <v-dialog v-model="FastReservationForm" max-width="auto" persistent>
      ...
      <v-card>
        <v-form ref="form" v-model="form" lazy-validation>
                ...
                <template v-slot:header>
                  <v-toolbar class="mb-2" color="indigo darken-5" dark flat>
                    <v-toolbar-title v-if="hotel !== null"
                      ><h1>
                        Rooms on discount in {{ hotel.name }}:
                      </h1></v-toolbar-title
                    >
                  </v-toolbar>
                </template>
                ...
                <template v-slot:item="props">
                  <v-flex xs12 sm6 md4 lg3>
                    <v-card>
                      <v-card-title
                        ><h2>
                          Room {{ props.item.room.roomNumber }} on
                          {{ props.item.room.hotelFloor.level }}. floor
                        </h2></v-card-title
                      >
                      <v-divider></v-divider>
                      <v-list dense>
                        <v-list-tile>
                          <v-list-tile-content
                            >Number of beds:</v-list-tile-content
                          >
                          <v-list-tile-content class="align-end"
                            ><h2>
                              {{ props.item.room.numberOfBeds }}
                        ...
                        <v-list-tile>
                          ...
                          <v-list-tile-content class="align-end"
                            ><h2>
                              {{ props.item.roomDiscount.startDate }}
                            </h2></v-list-tile-content
                          >
                        </v-list-tile>
                        <v-list-tile>
                          ...
                          <v-list-tile-content class="align-end"
                            ><h2>
                              {{ props.item.roomDiscount.endDate }}
                            </h2></v-list-tile-content
                          >
                        </v-list-tile>
                        <v-list-tile>
                          ...
                          <v-list-tile-content class="align-end"
                            ><h2>{{ props.item.roomDiscount.price }}</h2>
                            eur for the night</v-list-tile-content
                          >
                        </v-list-tile>
                      </v-list>
                      <v-card-actions>
                        <v-btn color="success" @click="reserve(props.item)"
                          >reserve
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="blue darken" flat @click="FastReservationForm = false"
              >Close</v-btn
            >
          </v-card-actions>
        </v-form>
...
export default {
  ...
  methods: {
    reserve(discountBean) {
      ...
      HotelController.reserve(this.$route.params.id, reservation).then(
        response => {
    ...
    findAvailableDiscounts(room) {
      ...
      room.roomDiscounts.forEach(discount => {
        ...
        room.roomTaken.forEach(taken => {
          ...
          if (
            endDiscountDate.getTime() < startTakenDate.getTime() ||
            startDiscountDate.getTime() > endTakenDate.getTime()
          ) {
            discountAvailable = false;
          }
          if (
            endDiscountDate.getTime() === endTakenDate.getTime() ||
