    ...
    <v-card>
      ...
      >
        <template v-slot:items="props">
          ...
          <td class="text-xs-left">{{ props.item.firstName }}</td>
          <td class="text-xs-left">{{ props.item.lastName }}</td>
          <v-btn color="primary" @click="withdraw(props.item.id)"
            >Withdraw</v-btn
          >
        </template>
      </v-data-table>
...
export default {
  ...
  beforeMount() {
    ...
    UserController.getFriendRequests(this.id).then(response => {
  ...
  methods: {
    accept(from) {
      ...
      FriendshipController.acceptFriendRequest(data).then(response => {
        let ind = -1;
        this.requestsFrom.forEach(element => {
          if (element.id === from) ind = this.requestsFrom.indexOf(element);
        });
        if (ind != -1) this.requestsFrom.splice(ind, 1);
    ...
    cancel(from) {
      ...
      FriendshipController.cancelFriendRequest(data)
        .then(response => {
          store.commit("setSnack", {
            msg: "You have successfully cancel friendship request",
            color: "success"
          });
          let ind = -1;
          this.requestsFrom.forEach(element => {
    ...
    withdraw(toId) {
      ...
      FriendshipController.cancelFriendRequest(data)
        .then(response => {
          store.commit("setSnack", {
            msg: "You have successfully withdrawn your friendship request",
            color: "success"
          });
          let ind = -1;
          this.requestsTo.forEach(element => {
            if (element.id === toId) ind = this.requestsTo.indexOf(element);
          });
          if (ind != -1) this.requestsTo.splice(ind, 1);
        })
        .catch(error => {
          store.commit("setSnack", {
            msg: error.response.data,
