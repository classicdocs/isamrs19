  ...
  <v-container fluid style="background-color: white">
    <v-layout row wrap>
      ...
      <v-flex lg1 md1 sm1 xs1>
        <v-btn color="primary" round @click="search"
      ...
      <v-flex lg12 md12 sm12 xs12>
        ...
        >
          <template v-slot:items="props">
            ...
            <v-btn color="primary" @click="viewProfil(props.item.id)"
              >View profil</v-btn
            >
          </template>
        </v-data-table>
...
export default {
  ...
  methods: {
    search() {
      ...
      UserController.searchFriends(this.id, { friend: this.searchParams }).then(
        response => {
          if (response.data.length === 0)
            store.commit("setSnack", {
              msg: "There is no search result!",
              color: "info"
            });
