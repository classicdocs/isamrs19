      ...
      <v-dialog v-model="addVehicle" persistent max-width="600px">
        ...
        <v-card>
          <v-form ref="form" v-model="valid" lazy-validation>
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="blue darken" flat @click="addVehicle = false"
                >Close</v-btn
              >
              <v-btn :disabled="!valid" color="success" @click="validate"
                >Add</v-btn
              >
              <v-btn color="error" @click="reset">Reset</v-btn> </v-card-actions
            >`
          </v-form>
        </v-card>
      </v-dialog>
      ...
      <v-list two-line>
        ...
        >
          <v-list-tile-content>
            <v-list-tile-title
              id="cartitle"
              v-html="vehicle.vehicleManufacturer + ' ' + vehicle.vehicleModel"
            ></v-list-tile-title>
            <v-layout>
              <v-flex>
              ...
              <v-flex>
                ...
                <v-btn
                  class="vehicleButtons"
                  color="error"
                  @click="showRemoveDialog(vehicle)"
    ...
    <v-dialog width="300" v-model="removeDialog">
      <v-card class="elevation-16 mx-auto">
        ...
        <v-card-actions class="justify-space-between">
          ...
          <v-btn color="error" flat @click="removeVehicle()">
            Remove
          </v-btn>
        </v-card-actions>
      </v-card>
    ...
    <v-dialog v-model="editDialog" persistent max-width="600px">
      <v-card>
        <v-form ref="editForm" v-model="editFormValid" lazy-validation>
          ...
          <v-card-text>
            <v-container grid-list-md>
              <v-layout wrap>
                ...
                <v-flex xs12 sm6 md4>
                  <number-input
                    v-model="vehicleToEdit.numberOfPassengers"
                    :min="1"
                    :max="max_passengers"
                    inline
                    center
                ...
                <v-flex xs12 sm6 md4>
                  <number-input
                    v-model="vehicleToEdit.yearOfProduction"
                    :min="1990"
                    :max="2019"
                    inline
                    center
                ...
                <v-flex xs12 sm6 md4>
                  <number-input
                    v-model="vehicleToEdit.pricePerDay"
                    :min="1"
                    :max="1000"
                    inline
                    center
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="blue darken" flat @click="cancelEditDialog"
              >Cancel</v-btn
            >
            <v-btn :disabled="!editFormValid" color="success" @click="edit"
              >Save changes</v-btn
            > </v-card-actions
          >`
        </v-form>
...
export default {
  ...
  data: () => ({
    ...
    modelSelected: false,
    car_models_list: [],
    vehicle_name_rules: [v => !!v || "Name is required"],
    vehicle_manufacturer_rules: [
      v => !!v || "Vehicle manufacturer is required"
    ],
    vehicle_model_rules: [v => !!v || "Vehicle model is required"],
    vehicle_type_rules: [v => !!v || "Vehicle type is required"],
    carType: "",
    passengers: null,
    production: 2012,
    price_per_day: 25,
  ...
  methods: {
    ...
    onSubmit() {
      ...
      VehicleController.create(this.vehicle).then(response => {
        this.vehicle_list.push(response.data);
        store.commit("setSnack", {
          msg: "You have successfully added a new vehicle.",
          color: "success"
        });
      });
      this.reset();
    ...
    removeVehicle() {
      ...
      VehicleController.remove(this.vehicleDelete)
        .then(response => {
          store.commit("setSnack", {
            msg: "You have successfully removed a vehicle.",
            color: "success"
          });
          let idx = this.vehicle_list.indexOf(this.VehicleToRemove);
          if (idx != -1) this.vehicle_list.splice(idx, 1);
        })
        .catch(error => {
          store.commit("setSnack", {
      ...
      if (this.$refs.editForm.validate()) {
        VehicleController.edit(this.vehicleToEdit)
          .then(response => {
            this.getData();
            store.commit("setSnack", {
              msg: "Vehicle successfully edited.",
              color: "success"
            });
          })
          .catch(error => {
