    ...
    filter(data) {
      ...
      this.searchResult.forEach(flight => {
        ...
        if (data.stops !== null) {
          ...
          if (f2 !== null) transfers += f2.transferDestinations.length;
        ...
        switch (this.searchParams.seatClass) {
          case "First": {
            ...
            if (f2 !== null) price += f2.ticketPriceFirst;
            if (!(minPrice <= price && price <= maxPrice)) return;
            break;
          }
          case "Business": {
            let price = f1.ticketPriceBusiness;
            if (f2 !== null) price += f2.ticketPriceBusiness;
            if (!(minPrice <= price && price <= maxPrice)) return;
            break;
          }
          case "Economy": {
            let price = f1.ticketPriceEconomy;
            if (f2 !== null) price += f2.ticketPriceEconomy;
            if (!(minPrice <= price && price <= maxPrice)) return;
            break;
          }
        }
        ...
        if (f2 !== null) time += f2.flightTimeHours;
        if (!(minTime <= time && time <= maxTime)) return;
