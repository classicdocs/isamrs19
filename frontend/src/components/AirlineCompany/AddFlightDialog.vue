<template>
  <div>
    <v-dialog v-model="addFormDialog" max-width="500px" persistent>
                  ...
                  >
                    ...
                    >
                      ...
                      <v-btn
                        ...
                        color="primary"
                        @click="
                          $refs.menuDepartureDate.save(flight.departureDate)
                        "
                        >OK</v-btn
                      >
                    </v-date-picker>
                ...
                <v-flex xs6>
                  ...
                  >
                    ...
                    <v-time-picker
                      ...
                      full-width
                      @click:minute="
                        $refs.menuDepartureTime.save(flight.departureTime)
                      "
                    ></v-time-picker>
                  </v-menu>
                </v-flex>
                  ...
                  >
                    ...
                    >
                      ...
                      <v-btn
                        flat
                        color="primary"
                        @click="menuLandingDate = false"
                        >Cancel</v-btn
                      >
                      <v-btn
                        flat
                        color="primary"
                        @click="$refs.menuLandingDate.save(flight.landingDate)"
                        >OK</v-btn
                      >
                    </v-date-picker>
                  </v-menu>
                ...
                <v-flex xs6>
                  ...
                  >
                    ...
                    <v-time-picker
                      ...
                      full-width
                      @click:minute="
                        $refs.menuLandingTime.save(flight.landingTime)
                      "
                    ></v-time-picker>
                  </v-menu>
                </v-flex>
...
export default {
  ...
  methods: {
    ...
    validate() {
      if (this.$refs.form.validate()) {
        if (this.validateDestinations()) {
          ...
          this.flight.startDestination.name = this.startDestination.substring(
            0,
            ind
          );
          this.flight.startDestination.zip = this.startDestination.substring(
            ind + 1,
            this.startDestination.length
          );

          ind = this.finalDestination.lastIndexOf(" ");
          this.flight.finalDestination.name = this.finalDestination.substring(
            0,
            ind
          );
          this.flight.finalDestination.zip = this.finalDestination.substring(
            ind + 1,
          ...
          this.$emit("snack", {
            msg: "Start,final and transfer destinations can't be the same!",
            color: "error"
          });
        }
      }
    ...
    validateDestinations() {
      if (this.startDestination === this.finalDestination) return false;
