  ...
  <div>
        ...
        <v-container fluid grid-list-md v-if="hotel !== null">
          ...
          <template v-slot:header>
            <v-toolbar
              ...
              flat
            >
              <v-toolbar-title v-if="hotel !== null"><h1>Defined prices for hotel {{hotel.name}}:</h1></v-toolbar-title>
              <v-spacer></v-spacer>
              <v-btn color="success" @click="openAddNewPrice">ADD NEW PRICE <v-icon right>add</v-icon></v-btn>
            </v-toolbar>
          </template>
              ...
              <v-card>
                ...
                <v-list dense>
                  <v-list-tile>
                    ...
                    <v-list-tile-content class="align-end"><h2>{{props.item.description}}</h2></v-list-tile-content>
                  </v-list-tile>
                  <v-list-tile>
                    <v-list-tile-content>Price:</v-list-tile-content>
                    <v-list-tile-content class="align-end"><h2>{{ props.item.price }}</h2> eur for the night</v-list-tile-content>
                  </v-list-tile>
                </v-list>
                <v-card-actions>
                  <v-btn color="success" @click="editPrice(props.item)">EDIT
                    <v-icon right>create</v-icon>
                  </v-btn>
                  <v-spacer></v-spacer>
                  <v-btn color="error" @click="deletePrice(props.item)">DELETE
          ...
          <v-card-text>
            <v-container grid-list-md>
              <v-layout row wrap align-center>
                ...
                <v-flex xs6  v-if="!isEdditing">
                  <v-select :items="getFreeTypes()" label="type" v-model="type" required :rules="typeRules" ></v-select>
                </v-flex>
                <v-flex xs6 v-if="isEdditing">
                ...
                <v-flex xs6>
                    <v-text-field v-model="description" label="description" :rules="[v => !!v || 'Description is required']"></v-text-field>
                </v-flex>
                ...
                <v-flex xs6>
                  <number-input v-model="price" :min="0" inline center controls :rules="[v => !!v || 'Price is required']">
                  </number-input>
                </v-flex>
    ...
    <v-dialog v-model="removeOffer" max-width="500px">
        <v-card>
          ...
          <v-card-text>
            <v-subheader >Are you sure you want to delete {{type}} service?</v-subheader>
          </v-card-text>
...
export default {
    ...
    hotel: function(){
      ...
      if(h != null){
        h.floors.forEach(floor => {
          floor.roomsOnFloor.forEach(singleRoom => {    /*Treba popraviti da radi sa DTO*/
  ...
  methods: {
    ...
    deleteOffer(){
      ...
      if(h != null){
        h.priceList.forEach(offer => {
            if(offer.type == this.type && offer.description == this.description && offer.price == this.price){
              var exists = false;
              h.floors.forEach(floor => {
              ...
              if(exists){
                this.showSnackbar({msg: "You can not delete offer that is used to define some existing room price.", color: "error"})
              ...
              .then(response => {
                  ...
                  if(hotelForStore !== null){
                    hotelForStore.priceList = JSON.parse(JSON.stringify(response.data));
                    store.commit('newHotel', hotelForStore);
                  }
                  this.showSnackbar({msg: "Offer is successfully deleted", color: "success"})
                  
              })
              .catch((response) => {
                  this.showSnackbar({msg: "Error! Something with update went wrong...", color: "error"})
              })
    ...
    saveOffer(){
      if (this.type!= null && this.description != null) {
        ...
        .then(response => {
            ...
            if(this.isEdditing){
              this.showSnackbar({msg: "Hotels offer is successfully edited", color: "success"})
            }else{
              this.showSnackbar({msg: "Hotels offer is successfully added", color: "success"})
            }
        })
        .catch((response) => {
              this.showSnackbar({msg: "Error! Something with update went wrong...", color: "error"})
