<template>
  <div>
    <v-dialog v-model="AddDiscountForm" max-width="auto" persistent>
      ...
      <v-stepper v-model="step">
        <v-stepper-header>
          <v-stepper-step :complete="step > 1" step="1"
            >Date, price, offer</v-stepper-step
          >

          <v-divider></v-divider>

          <v-stepper-step :complete="step > 2" step="2"
            >for room</v-stepper-step
          >
        </v-stepper-header>
        ...
        <v-stepper-items>
            ...
            <v-layout row wrap>
              <v-flex>
                ...
                <number-input
                  ...
                  center
                  controls
                  :rules="[v => !!v || 'Discount price is required']"
                ></number-input>
              </v-flex>
            </v-layout>
          ...
          <v-stepper-content step="2">
            ...
            <v-layout row wrap>
              <v-flex>
                <h1>
                  number: {{ pickedRoom.roomNumber }} floor:
                  {{ pickedRoom.hotelFloor.level }}
                </h1>
              </v-flex>
              <v-flex>
  ...
  methods: {
    finish() {
      ...
      if (this.hotel != null && this.pickedRoom.numberOfBeds != null) {
        this.hotel.priceList.forEach(offer => {
          if (offer.type == "OneBed" && this.pickedRoom.numberOfBeds == 1) {
            ...
            offer.type == "TwoBeds" &&
            this.pickedRoom.numberOfBeds == 2
          ) {
            pr = offer.price;
          } else if (
            offer.type == "ThreeBeds" &&
            this.pickedRoom.numberOfBeds == 3
          ) {
            pr = offer.price;
          } else if (
            offer.type == "FourBeds" &&
            this.pickedRoom.numberOfBeds == 4
          ) {
            pr = offer.price;
          } else if (
            offer.type == "FiveBeds" &&
            this.pickedRoom.numberOfBeds == 5
          ) {
            pr = offer.price;
          }
        });
      ...
      if (pr < roomDiscount.price) {
        this.showSnackbar({
          msg: "Discount price cant be higher than hotel room price",
          color: "error"
        });
        return;
      }
      ...
      HotelController.addRoomDiscount(
