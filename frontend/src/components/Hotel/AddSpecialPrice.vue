  ...
  <div>
    ...
    <v-card>
      ...
      <v-card-title primary-title>
        <span class="headline">Pick a room for which you want to define special price</span>
      </v-card-title>
    <v-card-text>
    ...
    <v-dialog v-model="definePrice" max-width="500px">
        <v-card>
          <v-card-title>
            <span class="headline">Define special price for {{pickedRoom.roomNumber}}</span>
          </v-card-title>

          <v-card-text>
              <v-layout row wrap>
                <v-flex xs6>
                    <v-subheader><span v-if="pickedRoom.specialPrices.length>0"><h3>Dates when room has special prices: </h3></span></v-subheader>
                </v-flex>
                <v-flex xs6>
                    <v-list>
                ...
                <v-layout col span>
                  <v-flex>
                    ...
                    <v-date-picker v-model="newPrice.startDate" no-title scrollable >
                      <v-btn flat color="primary" @click="menuStartDate = false" >Cancel</v-btn>
                      <v-spacer></v-spacer>
                      <v-btn flat color="primary" @click="$refs.menuStartDate.save(newPrice.startDate)">OK</v-btn>
                    </v-date-picker>
                    </v-menu> 
                  </v-flex>
                  <v-flex>
                    ...
                    <v-date-picker v-model="newPrice.endDate" no-title scrollable >
                        <v-btn flat color="primary" @click="menuEndDate = false" >Cancel</v-btn>
                        <v-spacer></v-spacer>
                        <v-btn flat color="primary" @click="$refs.menuEndDate.save(newPrice.endDate)">OK</v-btn>
                    </v-date-picker>
                    </v-menu>  
                  </v-flex>
              ...
              <v-layout col wrap>
                  <v-flex align-center>
                        ...
                        <number-input v-model="newPrice.price" :min="1" inline center controls></number-input>
                    </v-flex>
              </v-layout>
          </v-card-text>
