      ...
      <v-card max-width="500px">
        <v-form ref="form" v-model="formValid" lazy-validation>
              ...
              <v-layout wrap>
                ...
                <v-flex xs12>
                  ...
                  <v-select
                    ...
                    :items="myRentACars"
                    v-model="selected"
                    label="choose rent-a-car"
                    outline
                    :rules="rentacar_rules"
                    required
                  ></v-select>
                </v-flex>
              </v-layout>
          ...
          <v-card-actions>
            ...
            <v-btn :disabled="!formValid" color="success" @click="validateUser">
              Create</v-btn
            > </v-card-actions
          >`
        </v-form>
...
export default {
  ...
  data: () => ({
    ...
    address_rules: [v => !!v || "Address is required"],
    confirm_password_rules: [v => !!v || "Password confirmation is required"],
    radio_rules: [v => !!v || "You need to choose type of admin"],
    hotel_rules: [v => !!v || "You need to choose existing hotel from list"],
    airline_rules: [
          ...
          response.data.forEach(element => {
            if (element.name == this.selected) {
              ...
              SysAdminController.createHotelAdmin(hotelAdmin)
                .then(response => {
                  this.$emit("finished", {
                    msg: "Hotel administrator successfully added",
                    color: "success"
                  });
                })
                .catch(error => {
          ...
          response.data.forEach(element => {
            if (element.name == this.selected) {
              ...
              SysAdminController.createAirlineAdmin(airlineAdmin)
                .then(response => {
                  this.$emit("finished", {
                    msg: "Airline company administrator successfully added",
                    color: "success"
                  });
                })
                .catch(error => {
          ...
          response.data.forEach(element => {
            if (element.name == this.selected) {
              ...
              SysAdminController.createRentACarAdmin(rentACarAdmin)
                .then(response => {
                  this.$emit("finished", {
                    msg: "Rent-a-car administrator successfully added",
                    color: "success"
                  });
                })
                .catch(error => {
