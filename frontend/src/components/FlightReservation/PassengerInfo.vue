  ...
  <div>
    <v-container>
      <v-layout row wrap>
        ...
        <v-flex lg6 md6 sm12 xs12>
          <v-text-field label="Email" v-model="passenger.email"></v-text-field>
          <v-text-field label="Phone" v-model="passenger.phone"></v-text-field>
          <v-text-field
            label="Address"
  ...
  methods: {
    ...
    setLuggage() {
      this.luggage.forEach(l => {
        if (l.price === 0) this.selectedLuggage = l;
      });
    }
  }
};
